...
with ui.sidebar():
    ...
    ui.input_slider("date_range", "Filter by Date Range",
                ...
                max = string_to_date("2024-10-31"),
                value = [string_to_date(x) for x in ["2018-03-31", "2024-10-31"]])
...
with ui.navset_card_underline(title = "New Listings"):
    ...
    with ui.nav_panel("Plot", icon = icon_svg("chart-line")):
        ...
        def listings_plot():
            ...
            # Converting the "Date" column to datetime for filtering
            new_listings_df_for_viz["Date"] = pd.to_datetime(new_listings_df_for_viz["Date"], format="%m/%d/%Y")
            ...
            # Creating Visualization using Ployly
            fig = px.line(df, x="Date", y="Value", color="StateName")
            #fig.update_xaxes(title_text="", dtick="M12", tickformat="%Y")
            fig.update_xaxes(title_text="")
            fig.update_yaxes(title_text="")
            return fig
